...
- **Debug the graph path** with `uv run python scripts/live_test_graph.py <policy.pdf>` (in-memory backends) or `scripts/trigger_policy_flow.py` (Postgres + node trace).
...
- **Show the eval harness** (optional): `uv run python -m policydecoder.evals.run_all --all` prints per-agent scores against gold data — a nice credibility moment if you have 30 seconds.
